...
export default function EnergyBatteryApp() {
  ...
  const getFontForNameMemo = useCallback((name: string) => 
    getFontForName(name, nameFontsRef.current), []);

  const getEmojiForNameMemo = useCallback((name: string) => 
    getEmojiForName(name, nameFontsRef.current, [...names, ...students.map(s => s.name)]), 
    [names, students]);
  ...
  // Initialize AudioManager after a delay to not block first paint
  useEffect(() => {
  ...
  const setAllStudentsTo7 = () => {
    setStudents(prev => prev.map(student => ({ ...student, score: 7 })));
    playMarioSuccess();
  };

  // Trigger confetti and success melody when all students reach 7, and stop after 5 seconds
  useEffect(() => {
    if (allStudentsHave7 && students.length > 0) {
      setShowConfetti(true);
      playMarioSuccess(); // Play the course clear melody
      const timer = setTimeout(() => {
  ...
  }, [allStudentsHave7, students.length]);
            ...
            <div className="grid grid-cols-2 gap-x-12 gap-y-1">
              {names.map((name, i) => (
                <StudentRow 
                  ...
                  getFontForName={getFontForNameMemo}
        ...
        <div className="relative min-h-screen p-8">
          ...
          <div className="absolute left-[54%] top-4 transform -translate-x-1/2"><div className="flex flex-col items-center">
            ...
            <div className="flex justify-end mt-8">
              <Button 
                onClick={setAllStudentsTo7}
                variant="ghost" 
                size="sm"
                className="bg-gray-100 hover:bg-gray-200 text-gray-600 text-sm px-4 py-2 rounded-md transition-colors"
              >
                Set all to 7
              </Button>
            </div>
